/**
 * Admin: delete Realtime Database mob instance rows (`worldMobs/{worldId}/instances/...`).
 * Optionally clears pending melee hit pushes (`worldMobPlayerHits/{worldId}`), same as
 * `src/firebase/labyMobsRtdb.js` / DevTools mob cleanup.
 *
 * Uses Firebase Admin (bypasses client rules). Chunked deletes avoid `WRITE_TOO_BIG` on huge trees.
 *
 * Credentials + project ID: same as `scripts/clear-world-loot.mjs` /
 * `scripts/reset-laby-shared-world.mjs`:
 *   • `scripts/firebase-admin.local.json` (gitignored), or
 *   • `GOOGLE_APPLICATION_CREDENTIALS`, or
 *   • `gcloud auth application-default login`
 *
 * Database URL: `FIREBASE_DATABASE_URL` or `VITE_FIREBASE_DATABASE_URL` in `.env`, else default host.
 *
 * Usage:
 *   node scripts/clear-world-mobs-rtdb.mjs --dry-run
 *   node scripts/clear-world-mobs-rtdb.mjs
 *   node scripts/clear-world-mobs-rtdb.mjs --world-id my-world
 *   node scripts/clear-world-mobs-rtdb.mjs --instances-only
 *
 * npm:
 *   npm run rtdb:clear-mobs:dry
 *   npm run rtdb:clear-mobs
 */
import dns from 'node:dns/promises'
import { existsSync, readFileSync } from 'node:fs'
import { dirname, join, resolve } from 'node:path'
import process from 'node:process'
import { fileURLToPath } from 'node:url'
import admin from 'firebase-admin'

const __dirname = dirname(fileURLToPath(import.meta.url))
const REPO_ROOT = resolve(__dirname, '..')
const FIREBASE_ADMIN_LOCAL_SA = join(REPO_ROOT, 'scripts', 'firebase-admin.local.json')

const DEFAULT_WORLD_ID = 'fus-world-laby'

function tryReadFirebasercProjectId() {
  try {
    const raw = readFileSync(join(REPO_ROOT, '.firebaserc'), 'utf8')
    const j = JSON.parse(raw)
    const id = j?.projects?.default
    return typeof id === 'string' && id.trim() ? id.trim() : null
  } catch {
    return null
  }
}

function tryReadViteFromEnvFiles() {
  const out = { projectId: null, databaseURL: null }
  for (const name of ['.env.local', '.env']) {
    const p = join(REPO_ROOT, name)
    if (!existsSync(p)) continue
    try {
      const raw = readFileSync(p, 'utf8')
      for (const line of raw.split(/\r?\n/)) {
        const mProj = line.match(
          /^\s*VITE_FIREBASE_PROJECT_ID\s*=\s*(?:'([^']*)'|"([^"]*)"|([^\s#]+))/,
        )
        if (mProj && !out.projectId) {
          const v = (mProj[1] ?? mProj[2] ?? mProj[3] ?? '').trim()
          if (v) out.projectId = v
        }
        const mDb = line.match(
          /^\s*VITE_FIREBASE_DATABASE_URL\s*=\s*(?:'([^']*)'|"([^"]*)"|([^\s#]+))/,
        )
        if (mDb && !out.databaseURL) {
          const v = (mDb[1] ?? mDb[2] ?? mDb[3] ?? '').trim()
          if (v) out.databaseURL = v
        }
      }
    } catch {
      /* ignore */
    }
  }
  return out
}

const argv = new Set(process.argv.slice(2))
const dryRun = argv.has('--dry-run')
const instancesOnly = argv.has('--instances-only')

const worldIdIdx = process.argv.indexOf('--world-id')
const worldId =
  worldIdIdx >= 0 && process.argv[worldIdIdx + 1]
    ? String(process.argv[worldIdIdx + 1])
    : DEFAULT_WORLD_ID

const viteEnv = tryReadViteFromEnvFiles()

let projectId =
  process.env.FIREBASE_PROJECT_ID || process.env.GCLOUD_PROJECT || null
const credPath = process.env.GOOGLE_APPLICATION_CREDENTIALS
if (!projectId && credPath) {
  try {
    projectId = JSON.parse(readFileSync(resolve(credPath), 'utf8')).project_id || null
  } catch {
    /* ignore */
  }
}
if (!projectId && existsSync(FIREBASE_ADMIN_LOCAL_SA)) {
  try {
    projectId = JSON.parse(readFileSync(FIREBASE_ADMIN_LOCAL_SA, 'utf8')).project_id || null
  } catch {
    /* ignore */
  }
}
if (!projectId) projectId = tryReadFirebasercProjectId()
if (!projectId) projectId = viteEnv.projectId

if (!projectId) {
  console.error(
    'Could not determine Firebase project ID. Set FIREBASE_PROJECT_ID or add VITE_FIREBASE_PROJECT_ID to .env',
  )
  process.exit(1)
}

let databaseURL =
  process.env.FIREBASE_DATABASE_URL || viteEnv.databaseURL
if (!databaseURL) {
  databaseURL = `https://${projectId}-default-rtdb.firebaseio.com`
}

function resolveAdminCredential() {
  if (credPath) {
    const abs = resolve(credPath)
    if (!existsSync(abs)) {
      console.error(`GOOGLE_APPLICATION_CREDENTIALS not found: ${abs}`)
      process.exit(1)
    }
    return admin.credential.cert(JSON.parse(readFileSync(abs, 'utf8')))
  }
  if (existsSync(FIREBASE_ADMIN_LOCAL_SA)) {
    console.log('[clear-world-mobs-rtdb] Using scripts/firebase-admin.local.json')
    return admin.credential.cert(JSON.parse(readFileSync(FIREBASE_ADMIN_LOCAL_SA, 'utf8')))
  }
  return admin.credential.applicationDefault()
}

async function assertOauth2Resolvable() {
  try {
    await dns.lookup('oauth2.googleapis.com')
  } catch (err) {
    const c = err && err.code
    if (c === 'ENOTFOUND' || c === 'EAI_AGAIN' || c === 'ESERVFAIL') {
      console.error(
        `[clear-world-mobs-rtdb] DNS: cannot resolve oauth2.googleapis.com (${c}). Check network/VPN/DNS.`,
      )
    }
    throw err
  }
}

/** @returns {Promise<number>} remove() call count */
async function removeRtdbPathChunked(rtdb, pathStr) {
  const ref = rtdb.ref(pathStr)
  const snap = await ref.get()
  if (!snap.exists()) return 0

  let removed = 0
  const val = snap.val()
  if (val !== null && typeof val === 'object' && !Array.isArray(val)) {
    const keys = Object.keys(val)
    for (const key of keys) {
      removed += await removeRtdbPathChunked(rtdb, `${pathStr}/${key}`)
    }
  }
  await ref.remove()
  return removed + 1
}

if (!admin.apps.length) {
  admin.initializeApp({
    credential: resolveAdminCredential(),
    projectId,
    databaseURL,
  })
}

const instancesPath = `worldMobs/${worldId}/instances`
const hitsPath = `worldMobPlayerHits/${worldId}`

async function dryRunPeek(rtdb, pathStr, label) {
  const snap = await rtdb.ref(pathStr).get()
  console.log(`[dry-run] ${label} ${pathStr} exists=${snap.exists()}`)
  if (snap.exists() && typeof snap.val() === 'object' && snap.val() !== null) {
    const keys = Object.keys(snap.val())
    console.log(`[dry-run] child count ≈ ${keys.length} (showing up to 15 keys)`)
    console.log(keys.slice(0, 15).join(', ') + (keys.length > 15 ? ' …' : ''))
  }
}

async function main() {
  await assertOauth2Resolvable()
  console.log(`[clear-world-mobs-rtdb] project=${projectId}`)
  console.log(`[clear-world-mobs-rtdb] databaseURL=${databaseURL}`)
  console.log(`[clear-world-mobs-rtdb] worldId=${worldId}`)
  console.log(
    `[clear-world-mobs-rtdb] instancesOnly=${instancesOnly} dryRun=${dryRun}`,
  )

  const rtdb = admin.database()

  if (dryRun) {
    await dryRunPeek(rtdb, instancesPath, 'instances')
    if (!instancesOnly) {
      await dryRunPeek(rtdb, hitsPath, 'hits')
    }
    return
  }

  let nInst = await removeRtdbPathChunked(rtdb, instancesPath)
  console.log(
    nInst === 0
      ? `[clear-world-mobs-rtdb] ${instancesPath} was already empty or missing.`
      : `[clear-world-mobs-rtdb] cleared ${instancesPath} (${nInst} delete pass(es), chunked).`,
  )

  if (!instancesOnly) {
    const nHits = await removeRtdbPathChunked(rtdb, hitsPath)
    console.log(
      nHits === 0
        ? `[clear-world-mobs-rtdb] ${hitsPath} was already empty or missing.`
        : `[clear-world-mobs-rtdb] cleared ${hitsPath} (${nHits} delete pass(es), chunked).`,
    )
  }
}

main().catch((e) => {
  console.error(e)
  process.exit(1)
})

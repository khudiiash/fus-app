...
import { getTeacherJournalHistory } from '@/firebase/collections'
import { enrichTeacherJournalTransactions } from '@/composables/useTransactionFeed'
...
onMounted(async () => {
  loading.value = true
  try {
    if (!userStore.items.length) await userStore.fetchItems({ force: true })
    const raw = await getTeacherJournalHistory(auth.profile.id, 60)
    history.value = await enrichTeacherJournalTransactions(raw, auth.profile.id)
  } catch (e) {
    console.warn('[AwardHistoryView]', e)
    toastError(e?.message || 'Не вдалося завантажити історію')
    history.value = []
  } finally {
    loading.value = false
  }
...
const totalAwarded = computed(() =>
  history.value.filter((tx) => tx.type === 'award').reduce((s, tx) => s + (tx.amount || 0), 0),
)
</script>
  ...
  <div class="flex flex-col gap-4">
    <div>
      <div class="flex items-center gap-2 mb-0.5">
        ...
        <h1 class="text-2xl font-extrabold gradient-heading">Історія</h1>
      </div>
      <div class="flex flex-col gap-0.5 text-slate-500 text-sm">
        <span>Нарахування монет учням та предметні значки, які тобі передали.</span>
        <div class="flex flex-wrap items-center gap-1.5">
          <span>Всього нараховано:</span>
          <CoinDisplay :amount="totalAwarded" size="sm" />
        </div>
      </div>
    </div>
    ...
    <div v-else-if="history.length === 0" class="text-center py-16 text-slate-600">
      ...
      <div class="font-bold text-slate-500">Записів ще немає</div>
      <div class="text-xs mt-1 text-slate-600">Нарахування та значки з’являться тут автоматично</div>
    </div>

    <div v-else class="flex flex-col gap-2.5">
      <template v-for="tx in history" :key="tx.id">
        <!-- Вхідний предметний значок -->
        <HistoryTransactionCard
          v-if="tx.type === 'badge_sent'"
          :tx="tx"
        ...
        <!-- Власне нарахування монет учню -->
        <div
          v-else-if="tx.type === 'award'"

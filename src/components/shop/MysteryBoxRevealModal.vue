...
<template>
  ...
  >
    <div v-if="revealed" class="flex flex-col gap-5 min-h-[200px]">
      <!-- Одна сцена: коробка та призи в тому ж місці -->
      <div
        ref="stageRoot"
        class="relative w-full min-h-[min(52vh,320px)] flex items-center justify-center rounded-2xl border border-white/[0.06] bg-white/[0.02]"
      >
        <div
          ref="boxLayer"
          v-show="phase === 'box'"
        ...
        >
          <MysteryBoxSprite :rarity="boxRarity" :size="132" />
        </div>

        <div
          ref="stageReveal"
          v-show="phase !== 'box'"
          class="absolute inset-0 flex flex-col items-center justify-center px-4 py-6 text-center"
        >
          <template v-if="phase === 'item' && currentItem">
            <ItemModelThumb
              ...
              :width="192"
              :height="220"
              class="rounded-2xl border border-white/10 shadow-lg shadow-black/40"
            />
            <p class="mt-5 text-lg sm:text-xl font-extrabold text-slate-100 leading-tight max-w-[280px]">
              {{ currentItem.name }}
            </p>
          </template>

          <template v-else-if="phase === 'coins'">
            <p class="text-sm font-bold text-slate-500 uppercase tracking-wide mb-3">Монети</p>
            <div
              class="inline-flex items-center justify-center rounded-2xl px-8 py-6"
              style="background: rgba(251, 191, 36, 0.14)"
            >
              <CoinDisplay :amount="coinAmount" size="lg" show-sign />
            </div>
          </template>

          <template v-else-if="phase === 'empty'">
            <p class="text-base font-semibold text-slate-400 max-w-xs leading-relaxed">
          ...
          <template v-else-if="phase === 'done' && finalView">
            <template v-if="finalView.type === 'item'">
              ...
              <p v-if="itemCount > 1" class="mt-2 text-sm text-slate-500">
            ...
            <template v-else-if="finalView.type === 'coins'">
              ...
              <div
                ...
                style="background: rgba(251, 191, 36, 0.14)"
              >
                <CoinDisplay :amount="finalView.amount" size="lg" show-sign />
              </div>
              <p v-if="itemCount" class="mt-4 text-sm text-slate-500">
      ...
      <button
        type="button"
        class="w-full py-3 rounded-xl font-extrabold text-sm transition-all active:scale-[0.98] disabled:opacity-40 disabled:pointer-events-none"
